...
// Return 0 for no touch, 1 for Plus, 2 for Minus
int detect_plus_minus(unsigned short mid_x, unsigned short mid_y, unsigned short x_num, unsigned short y_num) {
	if (touch_rect(mid_x - 20, mid_y - 8, mid_x, mid_y + 8, x_num, y_num)) {
		return 1;
	} else if (touch_rect(mid_x, mid_y - 8, mid_x + 20, mid_y + 8, x_num, y_num)) {
		return 2;
	} else {
		return 0;
	}
