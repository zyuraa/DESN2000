...
unsigned char lcd_putChar_15x21(
    unsigned short x, 
    unsigned short y, 
    unsigned char ch)
{  
    ...
    /* Bounds check updated for 15x21 size */
    if((x > (DISPLAY_WIDTH - 15)) || (y > (DISPLAY_HEIGHT - 21))) {
        return 0;
    }
    ...
    for(i = 0; i < 7; i++) {
        ...
        for(ry = 0; ry < 3; ry++) {
            ...
            /* Iterate over 5 font columns */
            for(j = 0; j < 5; j++) {
                color = ((data & font_mask[j]) == 0) ? backgroundColor : foregroundColor;
                
                /* Repeat each column 3 times horizontally */
                for(rx = 0; rx < 3; rx++) {
                    lcd_point(x + (j * 3) + rx, y + (i * 3) + ry, color);
                }
            }
        }
    }
    ...
    unsigned char *pStr)
{
    while(1) {      
        ...
        if(lcd_putChar_15x21(x, y, *pStr++) == 0) {
            break;
        }
        x += 18;
    }
